...
class SearchController extends Controller
{
    ...
    public function getPreferenceMatches(Request $request)
    {
        ...
        foreach ($fields as $field => $meta) {
            ...
            if ($value) {
                $count = User::active()->regularUser()->whereHas('userProfile', function ($q) use ($field, $value, $user, $userAge) {
        ...
        if ($preferences->min_age || $preferences->max_age) {
            $count = User::active()->regularUser()->whereHas('userProfile', function ($q) use ($preferences, $user) {
                $q->where('is_profile_active', true);
                if ($user->userProfile && $user->userProfile->religion_id) {
                    $q->where('religion_id', $user->userProfile->religion_id);
                }
        ...
        if (is_array($preferences->preferred_locations) && count($preferences->preferred_locations) > 0) {
            ...
            $count = User::active()->regularUser()->whereHas('userProfile', function ($q) use ($locations, $user, $userAge) {
                $q->where('is_profile_active', true);
                if ($user->userProfile && $user->userProfile->religion_id) {
                    $q->where('religion_id', $user->userProfile->religion_id);
                }
        ...
        if ($cities->isNotEmpty()) {
            ...
            $query = User::active()->regularUser()->select('users.*')->join('user_profiles', 'users.id', '=', 'user_profiles.user_id')
        ...
        if ($myDistrict && (!is_array($preferences->preferred_locations) || !in_array($myDistrict, $preferences->preferred_locations))) {
            $count = User::active()->regularUser()->whereHas('userProfile', function ($q) use ($myDistrict, $user, $userAge) {
                $q->where('is_profile_active', true);
                if ($user->userProfile && $user->userProfile->religion_id) {
                    $q->where('religion_id', $user->userProfile->religion_id);
                }
        ...
        if ($user->userProfile && $user->userProfile->latitude && $user->userProfile->longitude) {
            ...
            $count = User::active()->regularUser()->join('user_profiles', 'users.id', '=', 'user_profiles.user_id')
                ->where('users.id', '!=', $user->id)
                ->whereDoesntHave('blockedBy', function($q) use ($user) {
                    $q->where('user_id', $user->id);
                })
        ...
        if ($preferences->min_height || $preferences->max_height) {
            $count = User::active()->regularUser()->whereHas('userProfile', function ($q) use ($preferences, $user, $userAge) {
                $q->where('is_profile_active', true);
                if ($user->userProfile && $user->userProfile->religion_id) {
                    $q->where('religion_id', $user->userProfile->religion_id);
                }
        ...
        if ($myTongue) {
            $count = User::active()->regularUser()->whereHas('userProfile', function ($q) use ($myTongue, $user, $userAge) {
                $q->where('is_profile_active', true);
                if ($user->userProfile && $user->userProfile->religion_id) {
                    $q->where('religion_id', $user->userProfile->religion_id);
                }
        ...
        $count = User::active()->regularUser()->where('created_at', '>=', now()->subDays(7))
            ->whereHas('userProfile', function ($q) use ($user, $userAge) {
                $q->where('is_profile_active', true);
                if ($user->userProfile && $user->userProfile->religion_id) {
                    $q->where('religion_id', $user->userProfile->religion_id);
